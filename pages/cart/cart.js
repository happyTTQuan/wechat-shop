...
Page({
  ...
  checkedItem: function (event) {
    ...
    if (!this.data.isEditCart) {
      util.request(api.CartChecked, { goodsIdList: that.data.cartGoods[itemIndex].goodsId, isChecked: that.data.cartGoods[itemIndex].checked ? 0 : 1 }, 'POST').then(function (res) {
        if (res.errno === 0) {
          console.log(res.data);
          that.setData({
            cartGoods: res.data.cartList,
  ...
  deleteCart: function () {
    ...
    let goodsIds = productIds.map(function (element, index, array) {
      if (element.checked == true) {
